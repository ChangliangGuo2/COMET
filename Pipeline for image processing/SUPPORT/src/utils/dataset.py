...
class DatasetSUPPORT_test_stitch(Dataset):
    def __init__(self, noisy_image, indices, load_to_memory=True,\
    ...
    def __getitem__(self, i):
        ...
        noisy_image = self.noisy_image[init_s:end_s,:,:]
        # noisy_image = self.noisy_image[:,init_h:end_h,init_w:end_w]
        ...
        return noisy_image, single_coordinate
...
def gen_train_dataloader(patch_size, patch_interval, batch_size, noisy_data_list):
    ...
    for noisy_data in noisy_data_list:
        noisy_image = torch.from_numpy(imread(noisy_data).astype(np.float32)).type(torch.FloatTensor)
